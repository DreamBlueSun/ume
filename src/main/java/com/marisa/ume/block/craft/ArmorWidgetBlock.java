...
public class ArmorWidgetBlock extends CraftBlock {

    public ArmorWidgetBlock() {
        super();
    }
    ...
    public MenuProvider getMenuProvider(@NotNull BlockState state, @NotNull Level level, @NotNull BlockPos pos) {
        return new SimpleMenuProvider((openContainerId, inventory, player) ->
                new ArmorWidgetMenu(openContainerId, inventory, ContainerLevelAccess.create(level, pos)),
                MutableComponent.create(new TranslatableContents("ArmorWidget")));
    }
    ...
    public void spawnAfterBreak(@NotNull BlockState state, @NotNull ServerLevel level, @NotNull BlockPos pos, @NotNull ItemStack stack, boolean p) {
        Containers.dropItemStack(level, pos.getX(), pos.getY(), pos.getZ(), ItemRegistry.ARMOR_WIDGET_BLOCK.get().getDefaultInstance());
